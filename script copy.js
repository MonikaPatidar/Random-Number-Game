...
var minval=document.getElementById("min").value;
    ...
    var NumDifference = parseInt(maxval) - parseInt(minval);
        if(minval >= maxval){
            alert("Min val is grater");
        }
        else if(NumDifference <= 9){
            alert("different of both no shoud be 10");
        }
        else{
        document.getElementById("demo").innerHTML= "Minimim Value  " +minval;
        document.getElementById("demo1").innerHTML="Maximum Value  " + maxval;
        var RNum= Math.floor(Math.random() * 27) + 1;
        console.log(RNum);
        console.log(minval);
        var RandomNum = parseInt(minval)+parseInt(RNum);
